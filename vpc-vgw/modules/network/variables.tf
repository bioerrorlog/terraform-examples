...
variable "public_subnet_01" {
  description = "CIDR block and AZ for the public subnet 01"
  type = object({
    cidr = string
    az   = string
  })
...
variable "public_subnet_02" {
  description = "CIDR block and AZ for the public subnet 02"
  type = object({
    cidr = string
    az   = string
  })
  default = { cidr = "10.0.2.0/24", az = "ap-northeast-1c" }
}
